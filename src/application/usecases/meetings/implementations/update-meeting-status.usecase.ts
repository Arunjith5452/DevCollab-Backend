...
@injectable()
export class UpdateMeetingStatusUseCase implements IExecute<{ meetingId: string, status: MeetingStatus, endTime?: Date }, void> {
    constructor(
        @inject(MEETING_TYPES.MeetingRepository) private readonly _meetingRepository: IMeetingRepository<MeetingEntity>
    ) { }

    async execute(data: { meetingId: string, status: MeetingStatus, endTime?: Date }): Promise<void> {
        await this._meetingRepository.updateStatus(data.meetingId, data.status, data.endTime);
    }
}
