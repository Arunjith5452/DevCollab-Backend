...
@injectable()
export class GetAllPlansUseCase implements IExecute<{ page?: number; limit?: number }, { plans: PlanResponseDTO[]; total: number }> {
    constructor(
    ...
    async execute(params?: { page?: number; limit?: number }): Promise<{ plans: PlanResponseDTO[]; total: number }> {
        const { data, total } = await this._planRepository.findAllPaginated(undefined, params?.page, params?.limit);
        return {
